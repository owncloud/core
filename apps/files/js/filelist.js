...
(function() {
	...
	FileList.prototype = {
		...
		initialize: function($el, options) {
			...
			this._rowTemplate = Handlebars.compile($('#filestable-row').html());
		},
		...
		_createRow: function(fileData, options) {
			var $tr, td, simpleSize, basename, extension, sizeColor,
			...
			if (fileData.extraData) {
				...
				context.extraData = fileData.extraData;
			}
			...
			var context = {
				...
				size: fileData.size,
				fileName: name,
				path: _.isUndefined(fileData.path) ? this.getCurrentDirectory() : fileData.path,
				mime: mime,
				mtime: mtime,
				etag: fileData.etag,
				permissions: fileData.permissions || this.getDirectoryPermissions(),
				mountType: fileData.mountType,
			...
			context.iconUrl = icon;

			// linkUrl
			if (type === 'dir') {
				context.linkUrl = this.linkTo(path + '/' + name);
			}
			else {
				context.linkUrl = this.getDownloadUrl(name, path);
			}
			...
			if (type !== 'dir' && name.indexOf('.') !== -1) {
				context.fileBaseName = name.substr(0, name.lastIndexOf('.'));
				context.extension = name.substr(name.lastIndexOf('.'));
			} else {
				context.fileBaseName = name;
			}

			// size column
			if (typeof(fileData.size) !== 'undefined' && fileData.size >= 0) {
				context.sizeDisplayText = humanFileSize(parseInt(fileData.size, 10), true);
				var sizeColor = Math.round(160-Math.pow((fileData.size/(1024*1024)),2));
				context.sizeColor = 'rgb(' + sizeColor + ',' + sizeColor + ',' + sizeColor + ')';
			} else {
				context.sizeDisplayText = t('files', 'Pending');
			}
			...
			context.mtimeTitle = OC.Util.formatDate(mtime);
			context.mtimeDisplayText = OC.Util.relativeModifiedDate(mtime);
			context.mtimeColor = 'rgb('+modifiedColor+','+modifiedColor+','+modifiedColor+')';

			$tr = $(this._rowTemplate(context));
