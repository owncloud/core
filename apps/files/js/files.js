...
>>>>>>> #6051 [files] [UX] Move file spinner
...
=======
		$(selectedFiles).each(function(i,elem) {
			//since our helper is a clone there is an added new data attribute for easier location of clone's origin
	...
	//options for file drag/drop
	//need to clean up stop/start fade actions
	var dragOptions={
		revert: 'invalid', revertDuration: 300,
		opacity: 0.7, zIndex: 100, appendTo: 'body', cursorAt: { left: 24, top: 18 },
		helper: createDragShadow, cursor: 'move',
		start: function(event, ui){
			...
			if($selectedFiles.length > 1){
				$selectedFiles.parents('tr').fadeTo('slow', 0.2);
			}
			else{
		...
		stop: function(event, ui) {
			var $selectedFiles = $('td.filename input:checkbox:checked');
			if($selectedFiles.length > 1){
				$selectedFiles.parents('tr').fadeTo('slow', 1);
			}
			else{
				$(this).fadeTo('slow', 1);
			}
