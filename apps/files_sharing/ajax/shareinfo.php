...
if (!$isWritable) {
	\OC\Files\Filesystem::addStorageWrapper('readonly', fn ($mountPoint, $storage) => new \OC\Files\Storage\Wrapper\PermissionsMask(['storage' => $storage, 'mask' => \OCP\Constants::PERMISSION_READ + \OCP\Constants::PERMISSION_SHARE]));
}
...
function getChildInfo($dir, $view, $sharePermissions) {
	...
	if (($sharePermissions & \OCP\Constants::PERMISSION_READ) !== \OCP\Constants::PERMISSION_READ) {
		return [];
	}
