		...
		if (!Filesystem::file_exists($path)) {
			...
			if (!empty($versions)) {
				foreach ($versions as $v) {
					\OC_Hook::emit('\OCP\Versions', 'preDelete', ['path' => $path . $v['version'], 'user' => $uid, 'trigger' => self::DELETE_TRIGGER_MASTER_REMOVED]);
					self::deleteVersion($view, $filename . '.v' . $v['version']);
					\OC_Hook::emit('\OCP\Versions', 'delete', ['path' => $path . $v['version'], 'user' => $uid, 'trigger' => self::DELETE_TRIGGER_MASTER_REMOVED]);
				}
			}
		}
	...
	public static function expireOlderThanMaxForUser($uid) {
		...
		if (!empty($toDelete)) {
			foreach ($toDelete as $version) {
				\OC_Hook::emit('\OCP\Versions', 'preDelete', ['path' => $version['path'].'.v'.$version['version'], 'user' => $uid, 'trigger' => self::DELETE_TRIGGER_RETENTION_CONSTRAINT]);
				self::deleteVersion($view, $version['path'] . '.v' . $version['version']);
				\OC_Hook::emit('\OCP\Versions', 'delete', ['path' => $version['path'].'.v'.$version['version'], 'user' => $uid, 'trigger' => self::DELETE_TRIGGER_RETENTION_CONSTRAINT]);
			}
		}
	}
	...
	public static function expire($filename, $uid) {
		...
		if ($config->getSystemValue('files_versions', Storage::DEFAULTENABLED)=='true' && $expiration->isEnabled()) {
			...
			foreach ($toDelete as $key => $path) {
				\OC_Hook::emit('\OCP\Versions', 'preDelete', ['path' => $path, 'user' => $uid, 'trigger' => self::DELETE_TRIGGER_QUOTA_EXCEEDED]);
				self::deleteVersion($versionsFileview, $path);
				\OC_Hook::emit('\OCP\Versions', 'delete', ['path' => $path, 'user' => $uid, 'trigger' => self::DELETE_TRIGGER_QUOTA_EXCEEDED]);
			...
			while ($availableSpace < 0 && $i < $numOfVersions) {
				...
				\OC_Hook::emit('\OCP\Versions', 'preDelete', ['path' => $version['path'].'.v'.$version['version'], 'user' => $uid, 'trigger' => self::DELETE_TRIGGER_QUOTA_EXCEEDED]);
				self::deleteVersion($versionsFileview, $version['path'] . '.v' . $version['version']);
				\OC_Hook::emit('\OCP\Versions', 'delete', ['path' => $version['path'].'.v'.$version['version'], 'user' => $uid, 'trigger' => self::DELETE_TRIGGER_QUOTA_EXCEEDED]);
