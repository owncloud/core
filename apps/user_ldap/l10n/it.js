OC.L10N.register(
    ...
    "By default the internal username will be created from the UUID attribute. It makes sure that the username is unique and characters do not need to be converted. The internal username has the restriction that only these characters are allowed: [ a-zA-Z0-9+_.@- ].  Other characters are replaced with their ASCII correspondence or simply omitted. On collisions a number will be added/increased. The internal username is used to identify a user internally. It is also the default name for the user home folder. It is also a part of remote URLs, for instance for all *DAV services. With this setting, the default behavior can be overridden. To do so, enter the user display name attribute in the following field. Leave it empty for default behavior. Changes will have effect only on newly mapped (added) LDAP users." : "Di base il nome utente interno sarà creato dall'attributo UUID. Si assicura che il nome utente sia unico e che i caratteri non debbano essere convertiti. Il nome utente interno ha la restrizione che solo questi caratteri sono consentiti: [a-zA-Z0-9+_.@-]. Altri caratteri sono rimpiazzati con i loro corrispondenti ASCII o semplicemente omessi. In caso di conflitti un numero sarà aggiunto/aumentato. Il nome utente interno è utilizzato per identificare un utente internamente. È inoltre il nome predefinito per la cartella home dell'utente. È anche una parte degli URL remoti, per esempio per tutti i servizi *DAV. Con questa impostazione, il comportamento predefinito può essere sovrascritto. Per fare ciò, inserire l'attributo del nome utente visualizzato nello spazio seguente. Lascialo vuoto per l'impostazione predefinita. le modifiche avranno effetto soltanto sui utenti LDAP associati (aggiunti).",
