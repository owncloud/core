...
class OtherContacts extends \Google\Service\Resource
{
   ...
   * token will get an error with an google.rpc.ErrorInfo with reason
   * "EXPIRED_SYNC_TOKEN". In the case of such an error clients should make a full
   * sync request without a `sync_token`. The first page of a full sync request
   * has an additional quota. If the quota is exceeded, a 429 error will be
   ...
   * See example usage at [List the user's other contacts that have
