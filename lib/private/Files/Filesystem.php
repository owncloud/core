...
class Filesystem {
	...
	*/
	 ...
	 * @return boolean
	 */
	private static function regexValidyCheck($regexToBeChecked, $callerMessage='') {
	...
	private static function checkRegexAgainstFolderOrFile($regexList, $path, $callerMessage='') {
		foreach ($regexList as $item) {                           // foreach given regex
			...
			@\preg_match($newItem, null);                         // regex validy check
			if (self::regexValidyCheck($item, $callerMessage)) {  // check if regex error occur
