...
class LoggingHelper
{
	...
	public static function getLogLevel ($ocPath) {
		...
		if ($result["code"] != 0) {
			throw new \Exception("could not get log level " . $result["stdOut"] . " " . $result["stdErr"]);
		}
	...
	public static function setLogLevel($ocPath, $logLevel) {
		...
		if ($result["code"] != 0) {
			throw new \Exception(
				"could not set log level " . $result ["stdOut"] . " " . $result ["stdErr"]
			);
		}
	}
	...
	public static function getLogBackend($ocPath) {
		...
		if ($result["code"] != 0) {
			throw new \Exception(
				"could not get log backend " . $result ["stdOut"] . " " . $result ["stdErr"]
			);
		}
	...
	public static function setLogBackend($ocPath, $backend) {
		...
		if ($result["code"] != 0) {
			throw new \Exception(
				"could not set log backend " . $result ["stdOut"] . " " . $result ["stdErr"]
			);
		}
	}
	...
	public static function getLogTimezone($ocPath) {
		...
		if ($result["code"] != 0) {
			throw new \Exception(
				"could not get log timezone " . $result ["stdOut"] . " " . $result ["stdErr"]
			);
		}
		$pregResult = preg_match(
	...
	public static function setLogTimezone($ocPath, $timezone) {
		$result = SetupHelper::runOcc(["log:manage", "--timezone=$timezone"], $ocPath);
		if ($result["code"] != 0) {
			throw new \Exception(
				"could not set log timezone " . $result ["stdOut"] . " " . $result ["stdErr"]
			);
		}
	}
	 ...
	 */
	public static function tailFile($filepath, $noOfLinesToRead = 1, $adaptive = true)
	{
