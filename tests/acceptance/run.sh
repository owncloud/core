...
then
	...
	export BEHAT_PARAMS='{"extensions" : {"Behat\\MinkExtension" : {"browser_name": "'${BROWSER}'", "base_url" : "'${TEST_SERVER_URL}'", "selenium2":{"capabilities": {"marionette":null, "browserName": "'${BROWSER}'", "browser": "'${BROWSER}'", '${BROWSER_VERSION_SELENIUM_STRING}${PLATFORM_SELENIUM_STRING}'"name": "'${CAPABILITIES_NAME_TEXT}'", "extra_capabilities": {"goog:chromeOptions": { "w3c": false, "args": ["--disable-gpu", "--disable-gpu-sandbox", "--disable-dev-shm-usage", "--no-sandbox", "--disable-accelerated-2d-canvas", "--disable-impl-side-painting", "--disable-accelerated-jpeg-decoding", "--test-type=ui", "--no-first-run", "--disable-extensions"] },'${EXTRA_CAPABILITIES}'}}, "wd_host":"http://'${SAUCE_CREDENTIALS}${SELENIUM_HOST}':'${SELENIUM_PORT}'/wd/hub"}}, "SensioLabs\\Behat\\PageObjectExtension" : {}}}'
fi
