...
Feature: caldav
  ...
  Scenario: Accessing a not existing calendar of another user
    When the administrator requests calendar "Alice/MyCalendar" using the new WebDAV API
  ...
  Scenario: Accessing a not shared calendar of another user
    ...
    When user "Alice" requests calendar "admin/MyCalendar" using the new WebDAV API
    Then the CalDAV HTTP status code should be "404"
    And the CalDAV exception should be "Sabre\DAV\Exception\NotFound"
    And the CalDAV message should be "Node with name 'MyCalendar' could not be found"
  ...
  Scenario: Creating a new calendar
    ...
    When user "Alice" requests calendar "Alice/MyCalendar" using the new WebDAV API
