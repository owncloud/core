...
Feature: sharing
	...
	Scenario Outline: Delete all group shares
		...
		When user "meta" deletes the last share using the sharing API
	...
	Scenario: orphaned shares
		...
		And user "meta" has shared folder "/common/sub" with user "user1"
		When user "meta" deletes folder "/common" using the WebDAV API
	...
	Scenario: deleting a file out of a share as recipient creates a backup for the owner
		...
		And as "meta" the file "/shared/shared_file.txt" should not exist
	...
	Scenario: deleting a folder out of a share as recipient creates a backup for the owner
		Given using OCS API version "1"
		And user "meta" has created a folder "/shared"
		And user "meta" has created a folder "/shared/sub"
		And user "meta" has moved file "/textfile0.txt" to "/shared/sub/shared_file.txt"
		And user "meta" has shared folder "/shared" with user "user1"
		When user "user1" deletes folder "/shared/sub" using the WebDAV API
		Then the HTTP status code should be "204"
		And as "user1" the folder "/shared/sub" should not exist
		And as "meta" the folder "/shared/sub" should not exist
		And as "meta" the folder "/sub" should exist in trash
		And as "meta" the file "/sub/shared_file.txt" should exist in trash
	...
	Scenario: sharee of a read-only share folder tries to delete the shared folder
		...
		And user "meta" has created a folder "/shared"
		And user "meta" has moved file "/textfile0.txt" to "/shared/shared_file.txt"
		And user "meta" has sent HTTP method "POST" to OCS API endpoint "/apps/files_sharing/api/v1/shares" with body
			| path        | shared |
			| shareWith   | user1  |
			| shareType   | 0      |
			| permissions | 1      |
	...
	Scenario: sharee of a upload-only shared folder tries to delete a file in the shared folder
		...
		When user "user1" deletes file "/shared/shared_file.txt" using the WebDAV API
		Then the HTTP status code should be "403"
		And as "meta" the file "/shared/shared_file.txt" should exist
	...
	Scenario: sharee of a upload-only share folder tries to delete his file in the folder
		...
		Then the HTTP status code should be "403"
		And as "meta" the file "/shared/textfile.txt" should exist
