...
Feature: move (rename) file
  ...
  Scenario Outline: Moving a file
    ...
    When user "meta" moves file "/welcome.txt" to "/FOLDER/welcome.txt" using the WebDAV API
  ...
  Scenario Outline: Moving and overwriting a file
    Given using <dav_version> DAV path
    When user "meta" moves file "/welcome.txt" to "/textfile0.txt" using the WebDAV API
    Then the HTTP status code should be "204"
    And the downloaded content when downloading file "/textfile0.txt" for user "meta" with range "bytes=0-6" should be "Welcome"
    Examples:
      | dav_version |
      | old         |
      | new         |
  ...
  Scenario Outline: Moving (renaming) a file to be only different case
    Given using <dav_version> DAV path
    When user "meta" moves file "/textfile0.txt" to "/TextFile0.txt" using the WebDAV API
    Then the HTTP status code should be "201"
    And as "meta" the file "/textfile0.txt" should not exist
    And the content of file "/TextFile0.txt" for user "meta" should be "ownCloud test text file 0" plus end-of-line
    Examples:
      | dav_version |
      | old         |
      | new         |
  ...
  Scenario Outline: Moving (renaming) a file to a file with only different case to an existing file
    Given using <dav_version> DAV path
    When user "meta" moves file "/textfile1.txt" to "/TextFile0.txt" using the WebDAV API
    Then the HTTP status code should be "201"
    And the content of file "/textfile0.txt" for user "meta" should be "ownCloud test text file 0" plus end-of-line
    And the content of file "/TextFile0.txt" for user "meta" should be "ownCloud test text file 1" plus end-of-line
    Examples:
      | dav_version |
      | old         |
      | new         |
  ...
  Scenario Outline: Moving (renaming) a file to a file in a folder with only different case to an existing file
    Given using <dav_version> DAV path
    When user "meta" moves file "/textfile1.txt" to "/PARENT/Parent.txt" using the WebDAV API
    Then the HTTP status code should be "201"
    And the content of file "/PARENT/parent.txt" for user "meta" should be "ownCloud test text file parent" plus end-of-line
    And the content of file "/PARENT/Parent.txt" for user "meta" should be "ownCloud test text file 1" plus end-of-line
    Examples:
      | dav_version |
      | old         |
      | new         |
  ...
  Scenario Outline: Moving a file to a folder with no permissions
    ...
    And user "user1" has created a share with settings
      ...
      | permissions | 1         |
      | shareWith   | meta     |
    When user "meta" moves file "/textfile0.txt" to "/testshare/textfile0.txt" using the WebDAV API
    Then the HTTP status code should be "403"
    When user "meta" downloads the file "/testshare/textfile0.txt" using the WebDAV API
  ...
  Scenario Outline: Moving a file to overwrite a file in a folder with no permissions
    ...
    When user "meta" moves file "/textfile0.txt" to "/testshare/overwritethis.txt" using the WebDAV API
    Then the HTTP status code should be "403"
  ...
  Scenario Outline: move file into a not-existing folder
    Given using <dav_version> DAV path
    When user "meta" moves file "/welcome.txt" to "/not-existing/welcome.txt" using the WebDAV API
  ...
  Scenario Outline: rename a file into an invalid filename
    Given using <dav_version> DAV path
    When user "meta" moves file "/welcome.txt" to "/a\\a" using the WebDAV API
  ...
  Scenario Outline: rename a file into a banned filename
    Given using <dav_version> DAV path
    When user "meta" moves file "/welcome.txt" to "/.htaccess" using the WebDAV API
    Then the HTTP status code should be "403"
    Examples:
      | dav_version |
      | old         |
  ...
  Scenario Outline: Checking file id after a move
    Given using <dav_version> DAV path
    And user "meta" has stored id of file "/textfile0.txt"
    When user "meta" moves file "/textfile0.txt" to "/FOLDER/textfile0.txt" using the WebDAV API
    Then user "meta" file "/FOLDER/textfile0.txt" should have the previously stored id
  ...
  Scenario Outline: Checking file id after a move between received shares
    ...
    And user "meta" has shared folder "/folderA" with user "user1"
    And user "meta" has shared folder "/folderB" with user "user1"
  ...
  Scenario Outline: Renaming a file to a path with extension .part should not be possible
    Given using <dav_version> DAV path
    When user "meta" moves file "/welcome.txt" to "/welcome.part" using the WebDAV API
    Then the HTTP status code should be "400"
