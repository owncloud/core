...
Feature: CORS headers
  ...
  Scenario Outline: no CORS headers should be returned when CORS domain does not match protocol of Origin header
    Given using OCS API version "<ocs_api_version>"
    And user "user0" has added "http://aphno.badal" to the list of personal CORS domains
    When user "user0" sends HTTP method "GET" to OCS API endpoint "<endpoint>" with headers
      | header | value |
      | Origin | https://aphno.badal |
    Then the OCS status code should be "<ocs-code>"
  ...
  Scenario Outline: no CORS headers should be returned when CORS domain does not match protocol of Origin header
    Given using OCS API version "<ocs_api_version>"
    And the administrator has added "http://aphno.badal" to the list of personal CORS domains
    When the administrator sends HTTP method "GET" to OCS API endpoint "<endpoint>" with headers
      | header | value |
      | Origin | https://aphno.badal |
    Then the OCS status code should be "<ocs-code>"
  ...
  Scenario Outline: no CORS headers should be returned when CORS domain does not match port of Origin header
    Given using OCS API version "<ocs_api_version>"
    And user "user0" has added "http://aphno.badal" to the list of personal CORS domains
    When user "user0" sends HTTP method "GET" to OCS API endpoint "<endpoint>" with headers
      | header | value |
      | Origin | http://aphno.badal:8080 |
    Then the OCS status code should be "<ocs-code>"
    And the HTTP status code should be "<http-code>"
    Then the following headers should not be set
      | Access-Control-Allow-Headers  |
