...
Feature: Sharing files and folders with internal groups
  ...
  Background:
    ...
    And the administrator adds system config key "user.search_min_length" with value "2" using the occ command
  ...
  Scenario: expiration date is disabled for sharing with users and groups, user shares file with another user
    Given user "user1" has uploaded file "filesForUpload/lorem.txt" to "/lorem.txt"
  ...
  Scenario: expiration date is enabled for sharing with groups, user shares file with a group
    Given user "user1" has uploaded file "filesForUpload/lorem.txt" to "/lorem.txt"
...
#    And parameter "shareapi_default_expire_date_group_share" of app "core" has been set to "yes"
    ...
    When the user changes expiration date for share of group "grp1" to "+7 days" in the share dialog
    ...
    Scenario: expiration date is enforced for group, user shares file
      Given user "user1" has uploaded file "filesForUpload/lorem.txt" to "/lorem.txt"
      And parameter "shareapi_default_expire_date_user_share" of app "core" has been set to "yes"
      # The above step should not have been required
      And parameter "shareapi_default_expire_date_group_share" of app "core" has been set to "yes"
  ...
  Scenario: expiration date is enforced for group, user shares  and tries to change expiration date more than allowed
    ...
    Then dialog should be displayed on the webUI
      | title               | content                                                    |
      | Error while sharing | Cannot set expiration date more than 3 days in the future |
    And the response when user "user1" gets the info of their last share should include
      | share_type        | group                |
      | file_target       | /lorem.txt           |
      | expiration        | +3 days              |
      | uid_owner         | user1                |
    ...
    Scenario: expiration date is enforced for group, user reshares received file with the group
      ...
      Then the expiration date input field should be visible for the group "grp1" in the share dialog
      And the expiration date input field should be "+3 days" for the group "grp1" in the share dialog
      Then the response when user "user1" gets the info of their last share should include
        | share_type        | group                |
        | file_target       | /lorem.txt           |
        | expiration        | +3 days              |
        | uid_owner         | user1                |
  ...
  Scenario: expiration date is enabled and enforced for group, user receives a share with expiration date and tries to reshare with expiration date in future than the original
    Given parameter "shareapi_default_expire_date_user_share" of app "core" has been set to "yes"
      # The above step should not have been required
    And parameter "shareapi_default_expire_date_group_share" of app "core" has been set to "yes"
    And parameter "shareapi_enforce_expire_date_group_share" of app "core" has been set to "yes"
    ...
    Then dialog should be displayed on the webUI
      | title               | content                                                    |
      | Error while sharing | Cannot set expiration date more than 30 days in the future |
    And the response when user "user1" gets the info of their last share should include
      | share_type        | group                |
      | file_target       | /lorem.txt           |
      | uid_owner         | user1                |
