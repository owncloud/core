...
Feature: sharing
	...
	Scenario: Uploading same file to a public upload-only share multiple times
		...
		Then the content of file "/FOLDER/test.txt" for user "meta" should be "test"
		And the content of file "/FOLDER/test (2).txt" for user "meta" should be "test2"

	Scenario: Uploading file to a public upload-only share that was deleted does not work
		Given user "meta" has created a share with settings
			| path        | FOLDER |
			| shareType   | 3      |
			| permissions | 4      |
		When user "meta" deletes file "/FOLDER" using the WebDAV API
	...
	Scenario: Check quota of owners parent directory of a shared file
		...
		And user "meta" has moved file "/welcome.txt" to "/myfile.txt"
		And user "meta" has shared file "myfile.txt" with user "user1"
	...
	Scenario: Uploading to a user shared folder with read/write permission when the sharer has unsufficient quota does not work
		...
		And user "meta" has created a share with settings
			...
			| permissions | 15         |
			| shareWith   | user1      |
		And the quota of user "meta" has been set to "0"
	...
	Scenario: Uploading file to a public shared folder with read/write permission when the sharer has unsufficient quota does not work
		When user "meta" creates a share using the sharing API with settings
			...
			| shareType   | 3      |
			| permissions | 15     |
		When the quota of user "meta" has been set to "0"
		Then publicly uploading a file should not work
