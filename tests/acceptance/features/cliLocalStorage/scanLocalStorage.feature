...
Feature: Scanning files on local storage
  ...
  Scenario Outline: Adding a folder to local storage, sharing with groups and running scan for specific group should add files for users of that group
    ...
    And user "user0" has been added to group "<groupname>"
    And user "user1" has been added to group "<groupname>"
    And user "user0" has created folder "/local_storage/folder1"
    And the administrator has set the external storage "local_storage" to be never scanned automatically
    And user "user0" has shared folder "/local_storage/folder1" with group "<groupname>"
    And the administrator has scanned the filesystem for all users
  ...
  Scenario: Adding a folder to local storage, sharing with groups and running scan for a list of groups should add files for users in the groups
    ...
    And user "user0" has created folder "/local_storage1/folder1"
    And user "user1" has created folder "/local_storage2/folder2"
    And user "user2" has created folder "/local_storage3/folder3"
    ...
    And user "user0" requests "/remote.php/dav/files/user0/local_storage1/folder1" with "PROPFIND" using basic auth
    Then the propfind result of user "user0" should not contain these entries:
      | /local_storage1/folder1/hello1.txt |
    When user "user1" requests "/remote.php/dav/files/user1/local_storage2/folder2" with "PROPFIND" using basic auth
    Then the propfind result of user "user1" should not contain these entries:
      | /local_storage2/folder2/hello2.txt |
    When user "user2" requests "/remote.php/dav/files/user2/local_storage3/folder3" with "PROPFIND" using basic auth
