...
Feature: get file info using PROPFIND
  ...
  Scenario: send PROPFIND requests to another user's webDav endpoints as normal user
    When user "Brian" sends "PROPFIND" request on these endpoints to get property "d:getetag" about user "Alice"
      | endpoint                                           |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/dav/files/%username%/PARENT            |
      | /remote.php/dav/files/%username%/PARENT/parent.txt |
  ...
  Scenario: send PROPFIND requests to webDav endpoints without any authentication
    When a user sends "PROPFIND" request on these endpoints with no authentication about user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
  ...
  Scenario: send PROPFIND requests to webDav endpoints using token authentication should not work
    ...
    When the user sends "PROPFIND" request on these endpoints using the generated app password about user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
  ...
  Scenario: send PROPFIND requests to webDav endpoints using app password token as password
    ...
    And a new browser session for "Alice" has been started
    And the user has generated a new app password named "my-client"
    When the user "Alice" sends "PROPFIND" request on these endpoints to get property "d:getetag" using the basic auth and generated app password about user "Alice"
      | endpoint                                           |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/dav/files/%username%/PARENT            |
      | /remote.php/dav/files/%username%/PARENT/parent.txt |
