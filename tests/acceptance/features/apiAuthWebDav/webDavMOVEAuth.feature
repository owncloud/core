...
Feature: MOVE file/folder
  ...
  Scenario: send MOVE requests to webDav endpoints without any authentication
    When a user sends "MOVE" request on these endpoints with body "doesnotmatter" and no authentication about user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
  ...
  Scenario: send MOVE requests to webDav endpoints using token authentication should not work
    ...
    When the user sends "MOVE" request on these endpoints using the generated app password about user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
