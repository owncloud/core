...
Feature: delete file/folder
  ...
  Scenario: send DELETE requests to webDav endpoints as normal user with wrong password
    When user "Alice" requests these endpoints with "DELETE" including body using password "invalid" then the status codes should be as listed
  ...
  Scenario: send DELETE requests to webDav endpoints as normal user with no password
    When user "Alice" requests these endpoints with "DELETE" including body using password "" then the status codes should be as listed
      | endpoint                                      | http-code | body          |
      | /remote.php/webdav/textfile0.txt              | 401       | doesnotmatter |
      | /remote.php/dav/files/Alice/textfile0.txt     | 401       | doesnotmatter |
      | /remote.php/webdav/PARENT                     | 401       | doesnotmatter |
  ...
  Scenario: send DELETE requests to another user's webDav endpoints as normal user
    When user "Brian" requests these endpoints with "DELETE" including body then the status codes should be as listed
      | endpoint                                      | http-code | body          |
      | /remote.php/dav/files/Alice/textfile0.txt     | 404       | doesnotmatter |
      | /remote.php/dav/files/Alice/PARENT            | 404       | doesnotmatter |
      | /remote.php/dav/files/Alice/PARENT/parent.txt | 404       | doesnotmatter |
  ...
  Scenario: send DELETE requests to webDav endpoints using app password token as password
    ...
    And a new browser session for "Alice" has been started
    And the user has generated a new app password named "my-client"
    When the user "Alice" requests these endpoints with "DELETE" using the basic auth and generated app password then the status codes should be as listed
      | endpoint                                      | http-code | body          |
      ...
      | /remote.php/dav/files/Alice/FOLDER            | 204       | doesnotmatter |
