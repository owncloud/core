...
Feature: get file info using POST
  ...
  Scenario: send POST requests to webDav endpoints as normal user with no password
    When user "Alice" requests these endpoints with "POST" including body using password "" then the status codes should be as listed
      | endpoint                                      | http-code | body          |
      | /remote.php/webdav/textfile0.txt              | 401       | doesnotmatter |
      | /remote.php/dav/files/Alice/textfile0.txt     | 401       | doesnotmatter |
      | /remote.php/webdav/PARENT                     | 401       | doesnotmatter |
  ...
  Scenario: send POST requests to another user's webDav endpoints as normal user
    When user "Brian" requests these endpoints with "POST" including body then the status codes should be as listed
      | endpoint                                       | http-code | body          |
      | /remote.php/dav/files/Alice/textfile1.txt      | 404       | doesnotmatter |
      | /remote.php/dav/files/Alice/PARENTS            | 404       | doesnotmatter |
  ...
  Scenario: send POST requests to webDav endpoints using app password token as password
    ...
    And a new browser session for "Alice" has been started
    And the user has generated a new app password named "my-client"
    When the user "Alice" requests these endpoints with "POST" using the basic auth and generated app password then the status codes should be as listed
      | endpoint                                      | http-code | body          |
      # this method is not available so gives 501
      | /remote.php/webdav/textfile0.txt              | 501       | doesnotmatter |
      | /remote.php/dav/files/Alice/textfile1.txt     | 501       | doesnotmatter |
      | /remote.php/dav/files/Alice/PARENT/parent.txt | 501       | doesnotmatter |
      | /remote.php/webdav/PARENT                     | 501       | doesnotmatter |
      | /remote.php/dav/files/Alice/FOLDER            | 501       | doesnotmatter |
