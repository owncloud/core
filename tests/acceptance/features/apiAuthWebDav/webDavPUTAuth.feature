...
Feature: get file info using PUT
  ...
  Scenario: send PUT requests to webDav endpoints as normal user with wrong password
    When user "Alice" requests these endpoints with "PUT" including body using password "invalid" then the status codes should be as listed
  ...
  Scenario: send PUT requests to webDav endpoints as normal user with no password
    When user "Alice" requests these endpoints with "PUT" including body using password "" then the status codes should be as listed
      | endpoint                                      | http-code | body          |
      | /remote.php/webdav/textfile0.txt              | 401       | doesnotmatter |
      | /remote.php/dav/files/Alice/textfile0.txt     | 401       | doesnotmatter |
      | /remote.php/webdav/PARENT                     | 401       | doesnotmatter |
  ...
  Scenario: send PUT requests to another user's webDav endpoints as normal user
    When user "Brian" requests these endpoints with "PUT" including body then the status codes should be as listed
      | endpoint                                       | http-code | body          |
      | /remote.php/dav/files/Alice/textfile1.txt      | 403       | doesnotmatter |
      | /remote.php/dav/files/Alice/PARENTS            | 403       | doesnotmatter |
      | /remote.php/dav/files/Alice/PARENTS/parent.txt | 404       | doesnotmatter |
  ...
  Scenario: send PUT requests to webDav endpoints using app password token as password
    ...
    And a new browser session for "Alice" has been started
    And the user has generated a new app password named "my-client"
    When the user "Alice" requests these endpoints with "PUT" using the basic auth and generated app password then the status codes should be as listed
      | endpoint                                      | http-code | body          |
      | /remote.php/webdav/textfile0.txt              | 204       | doesnotmatter |
      | /remote.php/dav/files/Alice/textfile1.txt     | 204       | doesnotmatter |
      | /remote.php/dav/files/Alice/PARENT/parent.txt | 204       | doesnotmatter |
      | /remote.php/webdav/PARENS                     | 201       | doesnotmatter |
      | /remote.php/dav/files/Alice/FOLDERS           | 201       | doesnotmatter |
      # this folder already exists so gives 409 - CONFLICT
      | /remote.php/dav/files/Alice/FOLDER            | 409       | doesnotmatter |
