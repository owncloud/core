...
Feature: get file info using PUT
  ...
  Scenario: send PUT requests to webDav endpoints as normal user with wrong password
    When user "Alice" sends "PUT" request on these endpoints with body "doesnotmatter" using password "invalid" about user "Alice"
      | endpoint                                           |
  ...
  Scenario: send PUT requests to webDav endpoints as normal user with no password
    When user "Alice" sends "PUT" request on these endpoints with body "doesnotmatter" using password "" about user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
  ...
  Scenario: send PUT requests to another user's webDav endpoints as normal user
    When user "Brian" sends "PUT" request on these endpoints with body "doesnotmatter" about user "Alice"
      | endpoint                                            |
  ...
  Scenario: send PUT requests to webDav endpoints using invalid username but correct password
    When user "usero" sends "PUT" request on these endpoints with body "doesnotmatter" using password of the user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
  ...
  Scenario: send PUT requests to webDav endpoints using valid password and username of different user
    When user "Brian" sends "PUT" request on these endpoints with body "doesnotmatter" using password of the user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
  ...
  Scenario: send PUT requests to webDav endpoints without any authentication
    When a user sends "PUT" request on these endpoints with body "doesnotmatter" and no authentication about user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
  ...
  Scenario: send PUT requests to webDav endpoints using token authentication should not work
    ...
    When the user sends "PUT" request on these endpoints using the generated app password about user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile0.txt     |
      | /remote.php/webdav/PARENT                          |
  ...
  Scenario: send PUT requests to webDav endpoints using app password token as password
    ...
    And a new browser session for "Alice" has been started
    And the user has generated a new app password named "my-client"
    When the user "Alice" sends "PUT" request on these endpoints with body "doesnotmatter" using the basic auth and generated app password about user "Alice"
      | endpoint                                           |
      | /remote.php/webdav/textfile0.txt                   |
      | /remote.php/dav/files/%username%/textfile1.txt     |
      | /remote.php/dav/files/%username%/PARENT/parent.txt |
