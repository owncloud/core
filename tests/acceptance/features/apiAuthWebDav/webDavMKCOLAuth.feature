...
Feature: get file info using MKCOL
  ...
  Scenario: send MKCOL requests to webDav endpoints as normal user with wrong password
    When user "Alice" requests these endpoints with "MKCOL" including body using password "invalid" then the status codes should be as listed
  ...
  Scenario: send MKCOL requests to webDav endpoints as normal user with no password
    When user "Alice" requests these endpoints with "MKCOL" including body using password "" then the status codes should be as listed
      | endpoint                                      | http-code | body          |
      | /remote.php/webdav/textfile0.txt              | 401       | doesnotmatter |
      | /remote.php/dav/files/Alice/textfile0.txt     | 401       | doesnotmatter |
      | /remote.php/webdav/PARENT                     | 401       | doesnotmatter |
  ...
  Scenario: send MKCOL requests to another user's webDav endpoints as normal user
    When user "Brian" requests these endpoints with "MKCOL" including body then the status codes should be as listed
      | endpoint                                      | http-code | body |
      | /remote.php/dav/files/Alice/textfile0.txt     | 403       |      |
      | /remote.php/dav/files/Alice/PARENT            | 403       |      |
      | /remote.php/dav/files/Alice/PARENT/parent.txt | 409       |      |
      | /remote.php/dav/files/Alice/does-not-exist    | 403       |      |
  ...
  Scenario: send MKCOL requests to webDav endpoints using app password token as password
    ...
    And a new browser session for "Alice" has been started
    And the user has generated a new app password named "my-client"
    When the user "Alice" requests these endpoints with "MKCOL" using the basic auth and generated app password then the status codes should be as listed
      | endpoint                                  | http-code |
      | /remote.php/webdav/newCol                 | 201       |
      | /remote.php/dav/files/Alice/newCol1       | 201       |
      | /remote.php/dav/files/Alice/PARENT/newCol | 201       |
      | /remote.php/webdav/COL                    | 201       |
      | /remote.php/dav/files/Alice/FOLDER/newCol | 201       |
