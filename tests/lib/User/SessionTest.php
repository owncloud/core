...
class SessionTest extends TestCase {
	...
	public function testGetUser() {
		...
		$userSession = new Session($this->logger, $this->config, $manager,
			$session, $this->timeFactory,
			$this->tokenProvider, $this->serviceLoader, $this->userSyncService);
	...
	public function testIsLoggedIn($isLoggedIn) {
		...
		$userSession = $this->getMockBuilder(Session::class)
			->setConstructorArgs([$this->logger, $this->config, $manager,
				$session, $this->timeFactory, $this->tokenProvider,
				$this->serviceLoader, $this->userSyncService])
			->setMethods([
				'getUser'
			])
			->getMock();
...
//			->getMock();
	...
	public function testValidateSessionDisabledUser() {
		...
		/** @var Session | \PHPUnit_Framework_MockObject_MockObject $userSession */
		$userSession = $this->getMockBuilder(Session::class)
			->setConstructorArgs([$this->logger, $this->config, $userManager,
				$session, $timeFactory, $tokenProvider,
				$this->serviceLoader, $this->userSyncService])
			->setMethods(['logout'])
			->getMock();
	...
	public function testValidateSessionNoPassword() {
		...
		$userSession = $this->getMockBuilder(Session::class)
			->setConstructorArgs([$this->logger, $this->config, $userManager,
				$session, $timeFactory, $tokenProvider, $this->serviceLoader,
	...
	public function testUpdateSessionTokenPassword() {
		...
		$tokenProvider = $this->createMock(IProvider::class);
		$userSession = new Session($this->logger, $this->config, $userManager,
			$session, $timeFactory,
			$tokenProvider, $this->serviceLoader, $this->userSyncService);
