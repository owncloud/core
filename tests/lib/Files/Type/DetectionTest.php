...
class DetectionTest extends \Test\TestCase {
	 ...
	 * On PHP 7.4 it warns and returns false, and that false became detectString()'s
	 * return value, so Bitmap's deny-list saw '' and admitted the content it exists to
	 * reject. (On PHP 8 the same call raises a TypeError instead, an \Error that escapes
	 * the handler in Bitmap::getThumbnail() and turns a missing preview into a 500.)
	 */
