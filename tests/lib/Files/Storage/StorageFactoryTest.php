...
class StorageFactoryTest extends TestCase {
	public function testSimpleWrapper() {
		...
		$instance->addStorageWrapper('dummy', function ($mountPoint, IStorage $storage, IMountPoint $mount) {
			$this->assertInstanceOf('\\' . \OC\Files\Storage\Temporary::class, $storage);
	...
	public function testRemoveWrapper() {
		$instance = new \OC\Files\Storage\StorageFactory();
		$mount = new MountPoint('\\' . \OC\Files\Storage\Temporary::class, '/foo', [[]], $instance);
		$instance->addStorageWrapper('dummy', fn ($mountPoint, IStorage $storage) => new DummyWrapper(['storage' => $storage]));
	...
	public function testWrapperPriority() {
		$instance = new \OC\Files\Storage\StorageFactory();
		$mount = new MountPoint('\\' . \OC\Files\Storage\Temporary::class, '/foo', [[]], $instance);
		$instance->addStorageWrapper('dummy1', fn ($mountPoint, IStorage $storage) => new DummyWrapper(['storage' => $storage, 'data' => 1]), 1);
		$instance->addStorageWrapper('dummy2', fn ($mountPoint, IStorage $storage) => new DummyWrapper(['storage' => $storage, 'data' => 100]), 100);
		$instance->addStorageWrapper('dummy3', fn ($mountPoint, IStorage $storage) => new DummyWrapper(['storage' => $storage, 'data' => 50]), 50);
