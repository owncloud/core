...
<div class="section" id="shareAPI">
	...
	<p id="publicLinkSettings"
	   class="indent <?php if ($_['allowLinks'] !== 'yes' || $_['shareAPIEnabled'] === 'no') p('hidden'); ?>">
	...
	<p id="setDefaultExpireDate"
	   class="double-indent <?php if ($_['allowLinks'] !== 'yes' || $_['shareDefaultExpireDateSet'] === 'no' || $_['shareAPIEnabled'] === 'no') p('hidden'); ?>">
		<?php p($l->t('Expire after ')); ?>
		<input type="text" name='shareapi_expire_after_n_days' id="shareapiExpireAfterNDays"
			   placeholder="<?php p('7') ?>"
	...
	<p class="<?php if ($_['shareAPIEnabled'] === 'no') p('hidden'); ?>">
		...
		<label
			for="onlyShareWithGroupMembers"><?php p($l->t('Restrict users to only share with users in their groups')); ?></label><br/>
	</p>
	...
	<p class="<?php if ($_['shareAPIEnabled'] === 'no') p('hidden'); ?>">
		...
		<label
			for="allowMailNotification"><?php p($l->t('Allow users to send mail notification for shared files')); ?></label><br/>
	</p>
	...
	<p id="selectExcludedGroups"
	   class="indent <?php if (!$_['shareExcludeGroups'] || $_['shareAPIEnabled'] === 'no') p('hidden'); ?>">
		<select
			class="groupsselect"
			id="excludedGroups" data-placeholder="groups"
			title="<?php p($l->t('Groups')) ?>" multiple="multiple">
			<?php foreach ($_["groups"] as $group): ?>
				<option value="<?php p($group['gid']) ?>" <?php if ($group['excluded']) {
					p('selected="selected"');
				} ?>><?php p($group['gid']); ?></option>
			<?php endforeach; ?>
		</select>
		<br/>
...
<div class="section">
	<form id="mail_settings">
		...
		<p><?php p($l->t('This is used for sending out notifications.')); ?> <span id="mail_settings_msg"
																				   class="msg"></span></p>
...
<div class="section">
	...
	<table id="log" class="grid">
		...
		foreach ($_['entries'] as $entry): ?>
			<tr>
				<td>
					<?php p(ucwords(strtolower($entry->level_name))); ?>
				</td>
				<td>
