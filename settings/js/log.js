...
OC.Log = {
	...
	levels: {'100': 'Debug', '200': 'Info', '300': 'Warning', '400': 'Error', '500': 'Fatal'},
	...
	addEntries: function (entries) {
		for (var i = 0; i < entries.length; i++) {
			...
			if (isNaN(entry.datetime.date)) {
				timeTd.text(entry.datetime.date);
			} else {
				timeTd.text(formatDate(entry.datetime.date * 1000));
			}
			row.append(timeTd);
			$('#log').append(row);
		}
